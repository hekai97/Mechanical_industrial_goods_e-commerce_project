...
public class ActionAddressDaoImpl implements ActionAddressDao {
	...
	public ActionAddress findAddrsById(Integer addrId) {
		// TODO 自动生成的方法存根
		String sql = "SELECT id,uid,name,phone,mobile,province,city,district,addr,zip"
	...
	public int findDefaultAddrByUserId(Integer userId) {
		...
		String sql="select count(id) as num from action_address where uid=? and default_addr=1";
		try {
	...
	public int insertAddress(ActionAddress addr) {
		// TODO Auto-generated method stub
		String sql="insert into action_address(uid,name,phone,"
				+ "mobile,province,city,district,addr,zip,default_addr,created,updated) values(?,?,?,?,?,?,?,?,?,?,?,?)";
		Object[] params= {
				addr.getUid(),addr.getName(),addr.getPhone(),addr.getMobile(),addr.getProvince(),addr.getCity(),
	...
	public List<ActionAddress> findAddrsByUserId(Integer userId) {
		...
		String sql = "SELECT id,uid,name,phone,mobile,province,city,district,addr,zip,"
				...
				+ "FROM action_address WHERE uid = ? and del_state=0 order by default_addr desc,updated desc";
		try {
	...
	public ActionAddress findDefaultAddr(Integer userId) {
		// TODO 自动生成的方法存根
		String sql = "SELECT id,uid,name,phone,mobile,province,city,district,addr,zip"
				+ ",default_addr,del_state,created,updated FROM action_address WHERE uid = ?"
