...
public class ActionProductServiceImpl implements ActionProductService {
	...
	private ActionProductListVo createProductListVo(ActionProduct actionProduct) {
		...
		vo.setIconUrl(actionProduct.getIconUrl());
		vo.setStatus(actionProduct.getStatus());
		vo.setIs_hot(actionProduct.getHot());
		//处理特殊属性
		vo.setStatusDesc(ConstUtil.ProductStatus.getStatusDesc(actionProduct.getStatus()));
		
		vo.setHotStatus(ConstUtil.HotStatus.getHotDesc(actionProduct.getHot()));
		
		vo.setProductCategory(actionParamsDao.findParamById(actionProduct.getProductId()).getName());
		
		vo.setPartsCategory(actionParamsDao.findParamById(actionProduct.getPartsId()).getName());
	...
	public SverResponse<String> saveOrUpdate(ActionProduct actionProduct) {
		...
		//修改时:如果没有重新上传图片，
		if(!StringUtils.isEmpty(actionProduct.getSubImages())) {
			String[] array=actionProduct.getSubImages().split(",");
			//拿出第一个元素作为主图
			actionProduct.setIconUrl(array[0]);
			//剩下的作为子图
			String temp=actionProduct.getSubImages();
			int index=temp.indexOf(",");
			if(index!=-1) {
				if(temp.substring(index+1).equals("null")) {
					actionProduct.setSubImages(null);
				}else {
					actionProduct.setSubImages(temp.substring(index+1));
				}
			}else {
				actionProduct.setSubImages(null);
			}
		...
		if(actionProduct.getId()!=null) {
			...
			actionProduct.setHot(ConstUtil.HotStatus.NORMAL_STATUS);
	...
	public SverResponse<PageBean<ActionProductListVo>> findProductsForPortal(Integer productTypeId, Integer partsId,
		...
		if(productTypeId != 0) {
			actionProduct.setProductId(productTypeId);
		}
		if(partsId != 0) {
			actionProduct.setPartsId(partsId);
		}
		//3.前端显示商品都为在售
